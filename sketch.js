...
function showTooltip(r) {
  ...
  let vCountry = data.getString(r, "Country") || "";
  let vType = data.getString(r, "Type") || "";
  let vStatus = data.getString(r, "Status") || "";
  let vElev = Number(data.getString(r, "Elevation (m)")) || 0;
...
function drawGrid() {
  ...
  for (let lon = minLon; lon <= maxLon; lon += 60) {
    let x = map(lon, minLon, maxLon, padding, width - padding);
    line(x, 100, x, height - padding); // grid più basso
  }
